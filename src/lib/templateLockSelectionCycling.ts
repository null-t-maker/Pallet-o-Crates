...
function tryBuildTemplateCycle(
  cycle: number,
  ...
  pallet: PalletInput,
): { placements: ParsedTemplatePlacement[]; score: number; weightAdded: number } | null {
  const eps = 1e-6;
...
export function buildSingleShapeTemplatePlacements(
  ...
  for (let cycle = 0; selectedCount < requestedQty; cycle++) {
    ...
    let bestCycle: { placements: ParsedTemplatePlacement[]; score: number; weightAdded: number } | null = null;
